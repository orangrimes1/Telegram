...
// stripped, and restore it. Strips ALL whitespace, not just leading/
// trailing — a bare .trim() doesn't catch a stray space typed between the
// dash and the digits (e.g. "- 1003905162486"), which is exactly what
// produced an invalid chat id here even after the first normalization fix.
...
// One-time startup log so the exact value actually being sent to Telegram's
// API is visible in Railway's logs, rather than inferred from a screenshot
// of the dashboard field or from log-line formatting — exactly the kind of
// ambiguity that made the earlier missing-minus-sign bug hard to pin down.
if (process.env.ADMIN_GROUP_CHAT_ID) {
  console.log('[adminGroup] Using chat ID:', normalizeChatId(process.env.ADMIN_GROUP_CHAT_ID));
}
